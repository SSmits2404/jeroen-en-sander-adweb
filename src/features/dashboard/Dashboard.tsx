...
export function Transactions() {
  ...
  if (!activeBudgetBookId) {
    return <p className="empty-text">Selecteer eerst een huishoudboekje via het tabblad "Huishoudboekjes".</p>;
  }
    ...
    <div>
      ...
      <div className="form-row">
        ...
        <div className="input-group">
          ...
          <select id="tx-type" value={type} onChange={(e) => setType(e.target.value as 'expense' | 'income')}>
        ...
        <button className="primary-button" type="button" onClick={handleCreate} disabled={!description.trim() || !amount}>
          Toevoegen
        </button>
      </div>
      ...
      <div className="card">
        ...
        {filteredTransactions.length === 0 && <p className="empty-text">Geen transacties deze maand.</p>}
            ...
            return (
              <li key={t.id} className={`transaction-item ${t.type}`}>
                <div>
                  ...
                  <p>{t.date} • {t.type === 'expense' ? 'Uitgave' : 'Inkomen'}</p>
                </div>
                <div className="amount-col">
                  <span className={`amount ${t.type}`}>
                    {t.type === 'expense' ? '-' : '+'}€{t.amount.toFixed(2)}
                  </span>
                  <button className="secondary-button small" type="button" onClick={() => handleDelete(t.id)}>
                    Verwijderen
                  </button>
                </div>
              </li>
